...
def create_faster_rcnn_model(features, scaled_gt_boxes, dims_input, cfg):
    ...
    fc_layers = clone_model(base_model, [cfg["MODEL"].POOL_NODE_NAME], [cfg["MODEL"].LAST_HIDDEN_NODE_NAME],
    ...
    detection_losses = create_detection_losses(cls_score, label_targets, bbox_pred, rois, bbox_targets,
...
def create_faster_rcnn_eval_model(model, image_input, dims_input, cfg, rpn_model=None):
    ...
    rpn = clone_model(model_with_rpn, [last_conv_node_name], ["rpn_cls_prob_reshape", "rpn_bbox_pred"],
                      CloneMethod.freeze)
    ...
    roi_fc_layers = clone_model(model, [last_conv_node_name, "rpn_target_rois"], ["cls_score", "bbox_regr"],
                                CloneMethod.freeze)
...
def train_faster_rcnn_alternating(cfg):
    ...
    if True:
        ...
        # train
        if debug_output: plot(stage1_rpn_network,
                              os.path.join(cfg.OUTPUT_PATH, "graph_frcn_train_stage1a_rpn." + cfg["CNTK"].GRAPH_TYPE))
    ...
    if True:
        ...
        # train
        if debug_output: plot(stage1_frcn_network,
                              os.path.join(cfg.OUTPUT_PATH, "graph_frcn_train_stage1b_frcn." + cfg["CNTK"].GRAPH_TYPE))
    ...
    if True:
        ...
        # train
        if debug_output: plot(stage2_rpn_network,
                              os.path.join(cfg.OUTPUT_PATH, "graph_frcn_train_stage2a_rpn." + cfg["CNTK"].GRAPH_TYPE))
        train_model(image_input, roi_input, dims_input, rpn_losses, rpn_losses,
                    rpn_lr_per_sample_scaled, mm_schedule, l2_reg_weight, rpn_epochs, cfg)
    ...
    if True:
        ...
        frcn = clone_model(stage1_frcn_network, [last_conv_node_name, "rpn_rois", "roi_input"],
                           ["cls_score", "bbox_regr", "rpn_target_rois", "detection_losses", "pred_error"],
                           CloneMethod.clone)
        ...
        # train
        if debug_output: plot(stage2_frcn_network,
                              os.path.join(cfg.OUTPUT_PATH, "graph_frcn_train_stage2b_frcn." + cfg["CNTK"].GRAPH_TYPE))
        train_model(image_input, roi_input, dims_input, detection_losses, pred_error,
                    frcn_lr_per_sample_scaled, mm_schedule, l2_reg_weight, frcn_epochs, cfg,
    ...
    return create_faster_rcnn_eval_model(stage2_frcn_network, image_input, dims_input, cfg,
